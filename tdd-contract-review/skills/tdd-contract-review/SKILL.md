...
└── outbound response field: PaymentGateway.charge — NO TESTS
    ├── ✗ success → assert params sent (amount, currency, user_id), db transaction.status == completed, db wallet.balance deducted
