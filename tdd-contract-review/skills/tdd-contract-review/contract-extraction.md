...
Other flags: `--depth N` (cap walk depth, default 7), `--format {json,markdown}` (output format, default `json`).

The walker writes the tree to `$RUN_DIR/tree__<file-slug>__<symbol-slug>.json` — the JSON is the source of truth (schema: `tdd-contract-review/schemas/call-tree.schema.json`). To get a human-readable indented view, render it via `scripts/render.py --kind call-tree --input <tree>.json --output <tree>.md`. Walk the JSON (or the rendered markdown) and paste the relevant subtree into the `### Call trees` fenced block of `01-extraction.json → contracts.call_tree_md`, then proceed with the algorithm below for any parts the walker flagged `[unresolved]` or `[depth-cap]` that need deeper inspection.
